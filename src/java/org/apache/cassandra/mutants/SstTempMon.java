...
public class SstTempMon {
    ...
    public static void StartMonitor(ColumnFamilyStore cfs, SSTableReader sstr) {
        if ((! DatabaseDescriptor.getMutantsOptions().migrate_sstables)
                || (! sstr.descriptor.mutantsTable))
            return;
    ...
    public static SSTableReader GetColdest() {
        ...
        if (coldestSstr != null)
            logger.warn("Mutants: GetColdest()={}", coldestSstr.descriptor.generation);
        return coldestSstr;
    }
    ...
    private static class Monitor {
        ...
        public void Stop() {
            ...
            try {
                ...
                logger.warn("Mutants: InterruptedException {}", e);
            }
            _mr = null;
        }
    ...
    private static class MonitorRunnable implements Runnable {
        ...
        static {
            sstMigrationDecisionIntervalMs = DatabaseDescriptor.getMutantsOptions().sst_migration_decision_interval_in_ms;
            logger.warn("Mutants: sstMigrationDecisionIntervalMs={}", sstMigrationDecisionIntervalMs);
        }
        ...
        public void Stop() {
            synchronized (_sleepLock) {
                //logger.warn("Mutants: gen={} stop requested", _sstr.descriptor.generation);
                _stopRequested = true;
                _sleepLock.notify();
            }
        }
        ...
        private static class AccessQueue {
            ...
            static {
                sstTempMonTimeWindowNs = DatabaseDescriptor.getMutantsOptions().sst_tempmon_time_window_in_sec
                    * 1000000000.0;
            }
            ...
            public boolean BelowColdnessThreshold(long curNano) {
                ...
                if (monitorDurNs < sstTempMonTimeWindowNs) {
                    //logger.warn("Mutants: Not enough time has passed. monitorDurNs={}", monitorDurNs);
                    return false;
                }

                return (((double)numAccessesInQ) / sstTempMonTimeWindowNs
                        < DatabaseDescriptor.getMutantsOptions().sst_tempmon_threshold_num_per_sec);
            }
        }

        public void run() {
            logger.warn("Mutants: TempMon Start {}", _sstr.descriptor.generation);
            long prevNnrd = -1;
            long prevNano = -1;
            ...
            while (! _stopRequested) {
                ...
                if (q.BelowColdnessThreshold(curNano)) {
                    ...
                    logger.warn("Mutants: TempMon TabletBecomeCold {}", _sstr.descriptor.generation);
            ...
            logger.warn("Mutants: TempMon Stop {}", _sstr.descriptor.generation);
        }
    }
}
